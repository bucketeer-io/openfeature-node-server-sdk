...
import {
  convertContextValueToString,
  evaluationContextToBKTUser,
} from '../src/internal/EvaluationContext';

describe('convertContextValueToString', () => {
  it('returns empty string for null and undefined', () => {
    expect(convertContextValueToString(null)).toBe('');
    const undefinedValue = undefined;
    expect(convertContextValueToString(undefinedValue as unknown as EvaluationContextValue)).toBe(
      '',
    );
  });
...
describe('evaluationContextToBKTUser', () => {
  ...
  it('handles null and undefined values', () => {
    ...
    const evaluationContext: EvaluationContext = {
      targetingKey: 'user-123',
      nullValue: null,
      undefinedValue: undefinedValue as unknown as EvaluationContextValue,
    };
